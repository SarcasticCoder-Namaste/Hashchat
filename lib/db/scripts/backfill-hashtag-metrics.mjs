...
for (const a of args) {
  if (a === "--full") {
    ...
    mode = "incremental";
    days = Math.max(1, parseInt(a.slice("--days=".length), 10) || 7);
  }
}
...
try {
  if (mode === "full") {
    console.log("[backfill-hashtag-metrics] FULL rebuild: clearing all rows...");
    await pool.query("DELETE FROM hashtag_metrics_daily");
  } else {
    console.log(
      `[backfill-hashtag-metrics] incremental rebuild from ${sinceDate} ` +
        `(deleting affected rows so re-aggregation is exact)...`,
    );
    await pool.query("DELETE FROM hashtag_metrics_daily WHERE day >= $1", [
      sinceDate,
    ]);
  }
  ...
  const sinceFilter =
    mode === "full"
      ? ""
      : `AND created_at >= '${sinceDate}T00:00:00Z'::timestamptz`;
  ...
  await pool.query(`
    ...
    WHERE m.room_tag IS NOT NULL
      ...
      ${sinceFilter.replace("created_at", "m.created_at")}
  ...
  await pool.query(`
    ...
    WHERE 1=1
      ${sinceFilter.replace("created_at", "uh.created_at")}
    GROUP BY uh.tag, day
  ...
  await pool.query(`
    ...
    WHERE 1=1
      ${sinceFilter.replace("created_at", "ufh.created_at")}
